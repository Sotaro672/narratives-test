...
export const onUserEmailVerified = onRequest({ cors: true }, async (req, res) => {
...
export const onAuthUserCreated = onRequest({ cors: true }, async (req, res) => {
  const { userId } = req.body;
...
// メール認証状態変更を検知するトリガー
export const checkEmailVerificationStatus = onRequest({ cors: true }, async (req, res) => {
  const { userId } = req.body;
...
// メール認証完了ユーザーの処理
async function processEmailVerifiedUser(userId: string) {
  try {
...
// メール認証完了後のウェルカムメール送信
async function sendPostVerificationWelcomeEmail(userId: string, userData: any, userRecord: admin.auth.UserRecord) {
  const db = admin.firestore();
...
export const onBusinessUserUpdated = onDocumentUpdated("business_users/{userId}", async (event) => {
...
export const narrativesApiProxy = onRequest({
